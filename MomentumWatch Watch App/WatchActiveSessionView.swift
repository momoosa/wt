    ...
    var body: some View {
        ZStack {
            ...
            Text(session.goal?.title ?? "Unknown")
                .font(.headline)
                .foregroundStyle((session.goal?.resolvedTheme ?? ThemeStore.defaultPreset).color(for: .dark))
            ...
            HStack {
                ZStack {
                    ...
                    Circle()
                        ...
                        .stroke(
                            (session.goal?.resolvedTheme ?? ThemeStore.defaultPreset).color(for: .dark),
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    .fill((session.goal?.resolvedTheme ?? ThemeStore.defaultPreset).color(for: .dark).opacity(0.2))
            )
            .onTapGesture {
                toggleTimer()
            }
