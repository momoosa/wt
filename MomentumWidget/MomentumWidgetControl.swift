...
extension MomentumWidgetControl {
    struct Value {
        var isRunning: Bool
        var name: String
    }
    ...
    struct Provider: AppIntentControlValueProvider {
        func previewValue(configuration: TimerConfiguration) -> Value {
            MomentumWidgetControl.Value(isRunning: false, name: configuration.timerName)
        }

        func currentValue(configuration: TimerConfiguration) async throws -> Value {
            let isRunning = true // Check if the timer is running
            return MomentumWidgetControl.Value(isRunning: isRunning, name: configuration.timerName)
        }
    }
}
