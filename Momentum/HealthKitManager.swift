...
public final class HealthKitManager {
    ...
    /// Fetch the total count for a count-based metric (e.g., steps) within a date range
    public func fetchCount(for metric: HealthKitMetric, from startDate: Date, to endDate: Date) async throws -> Double {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let query = HKStatisticsQuery(
                ...
                let sum = statistics?.sumQuantity()?.doubleValue(for: metric.unit) ?? 0
    ...
    /// Fetch count for a metric for today
